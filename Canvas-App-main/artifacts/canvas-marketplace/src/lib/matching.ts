// lib/matching.ts
// ─────────────────────────────────────────────────────────────
// Canvas AI Reference Photo Matching Engine — OPTIMIZED & FLEXIBLE
...
export interface ArtistTagIndex {
  id: string;
  aiTags?: AestheticTags;           // specialization tags
...
export interface ArtistMatchResult {
  artistId: string;
  score: number;                    // realistic percentage
  chips: string[];                  
...
const COVERAGE_BONUS_MAX = 8;       // Increased bonus for robust portfolios
const VERIFIED_BONUS = 3;
const INCOMPLETE_PENALTY = 12;
const MIN_SCORE = 0;               // Small baseline floor so valid artists don't bottom out at 0
const MAX_SCORE = 99;
const CHIP_SCORE_THRESHOLD = 50; 

// Increased cross-field credit so if a tag is classified under a different 
// bucket by OpenAI during search, it still awards high credit.
...
/** Highly flexible string matching with generous partial overlap */
export function matchStrings(a?: string, b?: string): number {
  ...
  if (ka.size === 0 || kb.size === 0) return 0.3; // Generous fallback instead of 0

  let shared = 0;
  for (const k of ka) if (kb.has(k)) shared++;
  if (shared === 0) return 0.2; // Generous baseline for any comparison
...
export function scoreArtistAgainstReference(ref: AestheticTags, artist: ArtistTagIndex): ArtistMatchResult {
  ...
  for (const field of fields) {
    ...
    if (best <= 0) best = 0.3; // Ensure every specified field gives a minimum baseline credit
  ...
  if (refTones.length > 0) {
    ...
    earned += TONES_WEIGHT * 0.8; // Generous tone matching credit
  ...
  if (imgs.length > 0) {
    ...
    coverage = 0.5; // Default healthy coverage for active profiles
  }
